...
import { ArrowRight, Activity, Zap, Upload, ArrowLeft, XCircle, History } from 'lucide-react';
...
export default function App() {
  ...
  const handleDeleteHistoryRow = async (id: number) => {
    try {
  ...
  if (showLanding) {
    return <Landing onTryNow={() => setShowLanding(false)} />;
  }
    ...
    <div className="app-container relative" style={{ padding: '40px 20px', maxWidth: '1200px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '30px', fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
        ...
        <button 
          ...
          className="flex items-center gap-2 hover:text-[#0033A0] transition-colors font-medium text-[#64748b]"
        >
          <History size={20} /> {showHistory ? 'Close History' : 'Screening History'}
        </button>
      </div>
